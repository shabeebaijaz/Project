    ...
    private void saveRecord() {
        ...
        if (validateName(sName)) {
            ...
            try {
                ...
                PreparedStatement ps = con.prepareStatement("insert into student(name,fname,mname,dob,gender,bloodg,mobile,email,address) values(?,?,?,?,?,?,?,?,?)");
                ...
                if (rs > 0) {
                    ...
                    PreparedStatement ps2 = con.prepareStatement("insert into edu values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    ps2.setInt(1, id);
                    ps2.setString(2, course);
                    ps2.setString(3, branch);
                    ps2.setString(4, hsYear);
                    ps2.setString(5, hsSchool);
                    ps2.setString(6, hsMM);
                    ps2.setString(7, hsMO);
                    ps2.setString(8, interYear);
                    ps2.setString(9, interSchool);
                    ps2.setString(10, interMM);
                    ps2.setString(11, interMO);
                    ps2.setString(12, gradYear);
                    ps2.setString(13, gradSchool);
                    ps2.setString(14, gradMM);
                    ps2.setString(15, gradMO);
                    ps2.setString(16, pgYear);
                    ps2.setString(17, pgSchool);
                    ps2.setString(18, pgMM);
                    ps2.setString(19, pgMO);
                    int rs2 = ps2.executeUpdate();
                    if (rs2 > 0) {
                        try {
                            File source = new File(photo);
                            File dest = new File("stu_pic/" + id + ".jpg");
                            Files.copy(source.toPath(), dest.toPath());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        JOptionPane.showMessageDialog(fr, "Record Inserted at ID: " + id);
                        resetAll();
                    } else {
                        JOptionPane.showMessageDialog(fr, "Education not Inserted", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                } else {
                    JOptionPane.showMessageDialog(fr, "Record not Inserted", "Error", JOptionPane.ERROR_MESSAGE);
                }
